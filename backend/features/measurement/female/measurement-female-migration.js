...
const db = new sqlite3.Database('./measurement-female-database.sqlite');

db.serialize(() => {
  // Create measurement female table
  ...
  ['client_name', 'measurement_date', 'size_number'].forEach(field => {
    db.run(`CREATE INDEX IF NOT EXISTS idx_femalemeasurement_${field} ON FemaleMeasurement(${field})`);
  });
